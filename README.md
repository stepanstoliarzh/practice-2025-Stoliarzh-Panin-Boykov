...
**Выводы по разведке**
Уязвимости:
  Цель использует nginx версии 1.19.0, на которой есть известные уязвимости. Это подтверждено Shodan.
  Использование Acunetix WVS указывает, что это тестовая площадка, но она может быть атакуемой при ослабленных настройках.
  Связи:
Через LeakIX обнаружена связь с доменом www.cam-cad.ru, который:
  Хостится в России (Selectel)
  Имеет SSL-сертификаты на несколько доменов
  Перенаправляет трафик на testphp.vulnweb.com
Инфраструктура:
  Домен работает на сервере Amazon EC2 (Oregon, США)
  IP-адрес: 44.228.249.3
Nmap
  есть только один открытый порт — HTTP (80/tcp)

Цель — тестовая система от Acunetix, размещённая в Amazon EC2.
Обнаружено 3 уязвимости в используемом веб-сервере nginx.
Прямая связь с российским сайтом cam-cad.ru (через редирект и сертификаты).
Используемые технологии — nginx, PHP, Ubuntu, без облачных защит или WAF.
Поддоменов немного, дополнительных сервисов не выявлено.
